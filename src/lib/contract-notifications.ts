...
export function renderContractInviteEmail(args: {
  ...
  const t = strings(args.language);
  const greeting = t.emailGreeting(args.recipientName);
  const lead = args.isReminder ? t.reminderLead : t.inviteLead(args.projectTitle);
  const subject = args.isReminder
    ? t.reminderSubject(args.contractTitle)
    : t.inviteSubject(args.contractTitle);
  ...
  const html = shell(`
    ...
    <p style="${P}">${escapeHtml(t.inviteHowTo)}</p>
    ${button(args.signingUrl, t.inviteButton)}
    <p style="${P}">${escapeHtml(t.inviteExpiry(args.expiresLabel))}</p>
    <p style="${P}">${escapeHtml(t.inviteCopyNote)}</p>
    <p style="margin:22px 0 0;color:#8a8580;font-size:12px;line-height:1.6;">${escapeHtml(t.inviteFallback)}<br />${escapeHtml(args.signingUrl)}</p>
  `);
  ...
  const text = [
    ...
    greeting,
    lead,
    `${t.inviteButton}: ${args.signingUrl}`,
    t.inviteExpiry(args.expiresLabel),
    t.inviteCopyNote,
    args.studioName,
  ].join("\n\n");
...
export function renderContractSignedEmail(args: {
  ...
  const t = strings(args.language);
  const greeting = t.emailGreeting(args.recipientName);
  const subject = t.signedSubject(args.contractTitle, args.projectTitle);

  const html = shell(`
    <h1 style="margin:0 0 6px;text-align:center;color:#202020;font-size:19px;letter-spacing:0.18em;text-transform:uppercase;">${escapeHtml(t.signedHeadline)}</h1>
    <p style="margin:0 0 26px;text-align:center;color:#77726a;font-size:12px;font-style:italic;">${escapeHtml(args.contractTitle)}</p>
    <p style="${P}">${escapeHtml(greeting)}</p>
    <p style="${P}">${escapeHtml(t.signedLead(args.signedAtLabel))}</p>
    <p style="${P}">${escapeHtml(t.signedKeep)}</p>
    ${args.downloadUrl ? button(args.downloadUrl, t.signedButton) : ""}
    <p style="margin:22px 0 0;color:#8a8580;font-size:11px;line-height:1.6;">${escapeHtml(t.signedHash)}<br /><span style="font-family:'Courier New',monospace;word-break:break-all;">${escapeHtml(args.pdfSha256)}</span></p>
  ...
  const text = [
    t.signedHeadline,
    greeting,
    t.signedLead(args.signedAtLabel),
    args.downloadUrl ? `${t.signedButton}: ${args.downloadUrl}` : null,
